...
flowchart LR
    ...
    MQTT -->|1. subscribe<br/>'robot/task_done'<br/>'robot/realtime'| S
    S -->|2. 데이터 큐 저장| Q
    Q -->|3. 비동기/배치 처리| R
...
2. **작업 완료 이력 적재 (Task History)**
   - 토픽: `robot/task_done`
   - 설명: 프론트엔드 HMI에서 로봇이 `Pick` 또는 `Place` 동작을 끝냈을 때 날아오는 성공 이벤트를 받아, MES(생산관리시스템) 통계를 위한 작업 이력 테이블에 `INSERT` 합니다.
