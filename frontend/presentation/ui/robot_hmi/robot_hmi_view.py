...
def _extract_first_program_tcp(data):
    """Return the first meaningful TCP found in Conty JSON data."""
    first_zero_tcp = None

    def _walk(obj):
        nonlocal first_zero_tcp
        if isinstance(obj, dict):
            target = obj.get("target")
            if isinstance(target, dict):
                tcp = _coerce_tcp(target.get("tcp"))
                if tcp:
                    if any(abs(v) > 1e-9 for v in tcp):
                        return tcp
                    if first_zero_tcp is None:
                        first_zero_tcp = tcp
            tcp = _coerce_tcp(obj.get("tcp"))
            if tcp:
                if any(abs(v) > 1e-9 for v in tcp):
                    return tcp
                if first_zero_tcp is None:
...
class ProgramTreeEditor:
    ...
    def __init__(self, parent_frame):
        ...
        self.pc_execution_speed_scale = 0.5
        self.program_motion_timeout_sec = 240.0
        self.program_signal_timeout_sec = 240.0
        self.dry_run_mode = False
        self.virtual_test_config = {}
    ...
    _TYPE_LABEL_MAP = {
        1: "Stop", 100: "Home", 102: "JointMove", 103: "FrameMove",
        200: "Pick Group", 201: "Pick", 202: "Place", 250: "Speed Ratio",
        901: "Call", 902: "Force", 903: "Comment",
    }

    def _insert_tree_item(self, parent, index="end", text="", node_type=None, open=True):
        """Insert a tree row with the compatibility color tag applied."""
        return self.tree.insert(parent, index, text=text, open=open, tags=(tag_for_type(node_type),))

    def _retag_tree_item(self, item, node_type=None):
        if hasattr(self, "tree") and item:
            self.tree.item(item, tags=(tag_for_type(node_type),))

    def _configure_compat_tree_tags(self):
        self.tree.tag_configure(SYSTEM_TAG, foreground=Theme.TEXT_PRIMARY)
        self.tree.tag_configure(STABLE_TAG, background="#D9F2D9", foreground=STABLE_TEXT_COLOR)
        self.tree.tag_configure(UNSTABLE_TAG, background="#4A4A4A", foreground=UNSTABLE_TEXT_COLOR)

    def _ensure_headless_tree(self):
        """Create only the tree widgets needed for Page 1 background execution."""
        if hasattr(self, "tree"):
            return
        self._headless = True
        self.tree = ttk.Treeview(self.parent, show="tree")
        self._configure_compat_tree_tags()

    def load_robot_program_for_execution(self, robot_name):
        """Load the selected robot's saved JSON into an execution tree without showing Page 2."""
        self.current_robot = robot_name or self.current_robot or "Robot A"
        self._ensure_headless_tree()
        self._sync_selected_robot()
        path = self.get_current_program_path(self.current_robot)
        if not os.path.exists(path):
            print(f">> [에러] {self.current_robot} 프로그램 파일을 찾을 수 없습니다: {path}")
            return False
    ...
    def is_execution_running(self):
        thread = getattr(self, "_program_thread", None)
                ...
                for child in self.tree.get_children(parent_item):
                    ...
                    if child in self.node_data and "__raw__" in self.node_data[child]:
                        ...
                        # 새로 만든 노드: 이름 기반 추측 매핑
                            ...
                            elif "Move C" in node_name: n.type = 106
                            elif "Move B" in node_name: n.type = 104
                            elif "Move By" in node_name: n.type = 105
                            elif "Move Home" in node_name: n.type = 100
                            ...
                            elif "Wait DI" in node_name: n.type = 28
                            elif "Wait For" in node_name: n.type = 23
                            ...
                            elif "Call" in node_name: n.type = 901
                            elif "Force" in node_name: n.type = 902
                            elif "TaktTime" in node_name or "indyCARE" in node_name: n.type = 302
                            elif "Folder" in node_name: n.type = 100
                            elif "Comment" in node_name: n.type = 903
                            elif "Stop" in node_name: n.type = 1
                            elif "Math" in node_name: n.type = 3
                            elif "Speed" in node_name: n.type = 250
                            elif "Tool Sensing" in node_name: n.type = 41
                            elif "Tool" in node_name: n.type = 40
                            elif "Conveyor" in node_name: n.type = 300
                            elif "Detect" in node_name: n.type = 400
                            elif "Retrieve" in node_name: n.type = 401
                            elif "Python Script" in node_name or "Python" in node_name: n.type = 50
                    ...
                    raw_templates = {
                        ...
                        3:   {"varList": [{"name": "var1", "type": 1, "value": 0}], "enable": True, "type": 3, "pId": p_id},
                        4:   {"doList": [], "enable": True, "type": 4, "pId": p_id},
                        5:   {"aoList": [], "enable": True, "type": 5, "pId": p_id},
                        6:   {"endtoolDoList": [], "enable": True, "type": 6, "pId": p_id},
                        20:  {"count": -1, "enable": True, "type": 20, "pId": p_id},
                        21:  {"enable": True, "type": 21, "pId": p_id},
                        22:  {"time": 1, "enable": True, "type": 22, "pId": p_id},
                        23:  {"time": 0, "cond": {"left": {"type": 10, "value": "var1"}, "right": {"type": 1, "value": 1}, "op": 0}, "enable": True, "type": 23, "pId": p_id},
                        24:  {"enable": True, "type": 24, "cond": {}, "pId": p_id},
                        25:  {"enable": True, "type": 25, "cond": {}, "pId": p_id},
                        26:  {"enable": True, "type": 26, "pId": p_id},
                        28:  {"endtoolDiList": [], "type": 28, "time": 1, "enable": True, "diList": [], "pId": p_id},
                        29:  {"endtoolDiList": [], "type": 29, "enable": True, "diList": [], "pId": p_id},
                        30:  {"endtoolDiList": [], "type": 30, "enable": True, "diList": [], "pId": p_id},
                        40:  {"toolCmd": {"toolId": 0, "cmdId": 0}, "enable": True, "type": 40, "pId": p_id},
                        41:  {"toolCmd": {"toolId": 0, "cmdId": 0}, "sensName": "sensor1", "enable": True, "type": 41, "pId": p_id},
                        50:  {"scriptFile": "script.py", "scriptCode": "", "enable": True, "type": 50, "pId": p_id},
                        100: {"enable": True, "type": 100, "pId": p_id},
                        102: {"enable": True, "type": 102, "name": "", "pId": p_id},
                        103: {"enable": True, "type": 103, "name": "", "pId": p_id},
                        104: {"enable": True, "type": 104, "pId": p_id},
                        105: {"enable": True, "type": 105, "pId": p_id},
                        106: {"enable": True, "type": 106, "pId": p_id},
                        200: {"groupName": "", "enable": True, "type": 200, "pId": p_id},
                    ...
                    202: {"enable": True, "type": 202, "pId": p_id, "toolId": 1, "sensName": "",
                          ...
                          "target": {"type": 0, "boundary": {"velLevel": 5, "accLevel": 5}, "pallet": {},
                                     "point": {"q": [], "p": []}, "refFrame": _ref, "tcp": _tcp}},
                        250: {"enable": True, "type": 250, "pId": p_id, "prgSpdRatio": 100},
                        300: {"enable": True, "type": 300, "pId": p_id},
                        302: {"enable": True, "type": 302, "pId": p_id, "careTackTime": 0, "targetTakt": 10.0},
                        400: {"enable": True, "type": 400, "pId": p_id},
                        401: {"enable": True, "type": 401, "pId": p_id},
                        901: {"enable": True, "type": 901, "pId": p_id, "subProgram": "sub_routine.json"},
                        902: {"enable": True, "type": 902, "pId": p_id},
                        903: {"enable": True, "type": 903, "pId": p_id},
                    }
    ...
    def _get_selected_robot_name(self):
        robot = None
        try:
            if hasattr(self, "robot_sel"):
                robot = self.robot_sel.get()
        except Exception:
            robot = None
        robot = robot or getattr(self, "current_robot", None) or "Robot A"
        self.current_robot = robot
        return robot

    def _sync_selected_robot(self):
        robot = self._get_selected_robot_name()
        try:
            robot_manager.set_active_robot(robot)
        except Exception:
            pass
    ...
    def _selected_robot_instance(self, robot_name=None):
        robot_name = robot_name or self._get_selected_robot_name()
        info = robot_manager.get_robot_info(robot_name)
    ...
    def _on_robot_changed(self, new_robot):
        ...
        if self.current_robot != new_robot:
            ...
            if os.path.exists(path):
                ...
                self._insert_tree_item("", text=" Main Program", node_type=None)
            self.refresh_info()
    ...
    def _load_from_path(self, file_path):
        if file_path and os.path.exists(file_path):
            try:
                for item in self.tree.get_children():
                    self.tree.delete(item)
                main_node = self._insert_tree_item("", text=" Main Program", node_type=None)
                ...
                if "nodes" in data and "program" not in data:
                    for n in data.get("nodes", []):
                        self._insert_tree_item(main_node, text=n if n.startswith(" ") else f" {n}", node_type=None)
                else:
                    from infrastructure.repositories.teaching_repository_impl import TeachingRepositoryImpl
                    repo = TeachingRepositoryImpl()
                    ...
                    self.current_program_tcp = _extract_first_program_tcp(self._current_raw_data)
                    if self.current_program_tcp:
                    ...
                    if hasattr(program, 'nodes'):
                        for node in program.nodes:
                            ...
                            pid = normalized_pid.get(cid, getattr(node, "pId", 0))
                            ...
                            if t == 999: node_str = " Program Settings"
                                ...
                                node_str = f" Speed ({spd}%)" if spd != "" else " Speed Ratio"
                            elif t == 302:
                                takt = getattr(node, "__raw__", {}).get("careTackTime", "")
                                node_str = f" TaktTime ({takt})" if takt != "" else " TaktTime"
                            elif t == 50:
                                node_str = f" Python ({name})" if name else " Python"
                            elif t == 901:
                                node_str = f" Call ({name})" if name else " Call"
                            elif t == 902:
                                node_str = f" Force ({name})" if name else " Force"
                            elif t == 903:
                                node_str = f" Comment ({name})" if name else " Comment"
                            ...
                            n_id = self._insert_tree_item(parent_item, text=node_str, node_type=t, open=False)
                            node_map[cid] = n_id
                            ...
                            if t in [102, 103, 104, 105, 106]:  # ★ Move 계열
                                ...
                                target = getattr(node, "target", None) or raw.get("target", {}) or {}
                                target_tcp = getattr(node, "target_tcp", None) or target.get("tcp", [0.0]*6)
                                target_ref = getattr(node, "target_refFrame", None) or target.get("refFrame", {"type": 1, "tref": [0,0,0,0,0,0]})
                                ...
                                self.node_data[n_id]["target"] = target
                                self.node_data[n_id]["tcp"] = target_tcp
                                self.node_data[n_id]["target_tcp"] = target_tcp
                                self.node_data[n_id]["refFrame"] = target_ref
                                self.node_data[n_id]["target_refFrame"] = target_ref
    ...
    def render(self):
        ...
        def _add_category(label, color, cmds):
            ...
            for cmd in cmds:
                if hasattr(cmd, "ui_name"):
                    cmd_name, col, stable = cmd.ui_name, cmd.color, cmd.stable
                else:
                    cmd_name, col, stable = cmd[0], cmd[1], True
                btn = ctk.CTkButton(left, text=f"  {cmd_name}", fg_color=col, height=26,
                                    text_color=text_color_for_stability(stable),
        ...
        btn_mv = ctk.CTkButton(left, text="  + 사용자 모션 변수", fg_color="#2E7D32", hover_color="#388E3C",
                                height=28, font=Theme.font(size=11, weight="bold"), anchor="w", text_color="black",
                                command=self._add_user_motion_var)
        btn_mv.pack(fill="x", padx=10, pady=1)
        for label, color, commands in COMMAND_SECTIONS:
            if label == "▸ 모션 명령어":
                _add_category("", color, commands)
                continue
            _add_category(label, color, commands)
        ...
        def _on_tree_mousewheel(event):
            ...
            if event.state & 0x0001:  # Shift + wheel: horizontal scroll
        ...
        self.tree.bind("<Shift-MouseWheel>", _on_tree_mousewheel)

        def _on_tree_scroll_up(event):
            self.tree.yview_scroll(-1, "units")
            return "break"

        def _on_tree_scroll_down(event):
            self.tree.yview_scroll(1, "units")
            return "break"

        self.tree.bind("<Button-4>", _on_tree_scroll_up)
        self.tree.bind("<Button-5>", _on_tree_scroll_down)
        ...
        self.pp_editor = PickPlaceEditor(self.pp_frame, robot_name_provider=self._get_selected_robot_name)
        ...
        self.jog_controller = JogController(self.jog_frame, robot_name_provider=self._get_selected_robot_name)
        ...
        def _on_node_selected(q, p, t_type, item_text, p_name=None, p_data=None, all_pallets=None, b_radius=0.0, app_data=None, ret_data=None, d=None):
            ...
            if conty_type in [102, 103]:  # ★ JointMove / FrameMove
            ...
            elif conty_type == 1:  # Stop
                _clear_pp()
                self.current_editor = self.stop_editor
                ...
                rv = cond.get("right", {}).get("value", 1) if cond else 1
                self.waitfor_editor.update_ui(item_text, "변수 조건", lv, op, rv, d.get("time", 0))
            ...
            elif conty_type in [32, 250]:  # SpeedRatio
                _clear_pp()
            ...
            elif conty_type == 901:  # Call placeholder
                _clear_pp()
                self.current_editor = self.call_editor
                self.call_editor.render()
                self.call_editor.update_ui(item_text, "sub_routine.json")

            elif conty_type == 302:  # TaktTime
                _clear_pp()
                self.current_editor = self.takttime_editor
                self.takttime_editor.render()
                self.takttime_editor.update_ui(item_text, d.get("careTackTime", raw.get("careTackTime", 0)), d.get("targetTakt", raw.get("targetTakt", 10.0)))

            elif conty_type == 902:  # Force placeholder
                _clear_pp()
                self.current_editor = self.force_editor
                self.force_editor.render()
                self.force_editor.update_ui(item_text, 500.0, 100.0, 50.0)

            elif conty_type == 903:  # Comment placeholder
                _clear_pp()
                self.current_editor = self.comment_editor
                self.comment_editor.render()
                self.comment_editor.update_ui(item_text)

            elif conty_type == 50:  # Exec Python
                _clear_pp()
                self.current_editor = self.python_editor
                self.python_editor.render()
                self.python_editor.update_ui(item_text, d.get("scriptFile", raw.get("scriptFile", "script.py")), d.get("scriptCode", raw.get("scriptCode", "")))
            ...
            elif conty_type == 3:  # Var assignment / Math
                _clear_pp()
                self.current_editor = self.math_editor
                self.math_editor.render()
                var_list = d.get("varList", raw.get("varList", []))
                first = var_list[0] if var_list else {}
                self.math_editor.update_ui(item_text, first.get("name", "var1"), "=", first.get("value", 1.0))

            elif conty_type == 2:  # Variables
                _clear_pp()
                self.current_editor = self.folder_editor
                self.folder_editor.render()
                self.folder_editor.update_ui(item_text)
    ...
    def _on_move_btn_clicked(self):
        ...
        robot_name = self._sync_selected_robot()
        selected = self.tree.selection()
        if not selected: return
        item = selected[0]
        item_text = self.tree.item(item, "text").strip()
        ...
        if "Move Home" in item_text or "Home" in item_text:
            ...
            RobotControlUseCase.move_to_joint([0.0, 0.0, -90.0, 0.0, -90.0, 0.0], robot_name)
            return
        ...
        if wps:
            ...
            self.jog_controller.update_coordinates(q, p)
            RobotControlUseCase.move_to_joint(q, robot_name)
        ...
        def _run_cycle():
            try:
                ...
                for i, wp in enumerate(wps):
                    ...
                    RobotControlUseCase.move_to_joint(q, robot_name)
                    # 이동 완료 대기
                    RobotControlUseCase.wait_for_move_finish(name=robot_name)
    ...
    def _stop_execution(self):
        ...
        RobotControlUseCase.request_stop(robot_name)  # wait_for_move_finish 즉시 반환
        try:
            inst = self._selected_robot_instance(robot_name)
            if inst:
                inst.stop_emergency()
        except:
            pass
    ...
    def _open_config_dialog(self):
        ...
        RobotConfigDialog(self.parent, robot_name_provider=self._get_selected_robot_name)
    ...
    def _add_user_motion_var(self):
        ...
        # 로봇 연결 시 현재 좌표 표시
        try:
            active = self._get_selected_robot_name()
            if active:
    ...
    def _run_program(self):
        ...
        if getattr(self, "_program_thread", None) and self._program_thread.is_alive():
            ...
            return
        self._exec_stop = False
        self._exec_ng_reason = None
        self._last_applied_tcp = None
        self._last_joint_vel_level = None
        self._last_task_vel_level = None
        exec_robot = self._sync_selected_robot()
        RobotControlUseCase.clear_stop_request(exec_robot)
        if not self._selected_robot_instance(exec_robot):
            print(f">> [오류] 선택된 {exec_robot} 로봇이 연결되지 않았습니다.")
            return
        print(f">> [실행 대상] 프로그래밍 화면 선택 로봇: {exec_robot}")
        dry_run = bool(getattr(self, "dry_run_mode", False))
        virtual_cfg = dict(getattr(self, "virtual_test_config", {}) or {})
        virtual_session_id = str(virtual_cfg.get("session_id") or "").strip()
        is_virtual_test = bool(virtual_session_id)
        try:
            virtual_target_cycles = int(virtual_cfg.get("target_cycles", 1))
        except (TypeError, ValueError):
            virtual_target_cycles = 1
        try:
        ...
        class _ProgramAbortException(Exception):
            """Fault/N.G/user stop으로 프로그램 실행을 즉시 중단하기 위한 예외"""
            pass

        def _activate_exec_robot():
            try:
                robot_manager.set_active_robot(exec_robot)
            except Exception:
                pass
            return exec_robot

        def _exec_inst():
            info = robot_manager.get_robot_info(exec_robot)
            return info.get("instance") if info else None

        def _safe_publish(topic, payload):
            try:
                mqtt_broker.publish(topic, payload)
            except Exception:
                pass

        def _notify_virtual(event, cycle_index=0, status="running", extra=None):
            ...
            payload = {
                ...
                "target_cycles": virtual_target_cycles,
                "sample_interval_ms": virtual_sample_interval_ms,
                "program_path": self.get_current_program_path(exec_robot),
                "dry_run": dry_run,
                "created_at": datetime.datetime.now().isoformat(timespec="milliseconds"),
            }
            if isinstance(extra, dict):
                payload.update(extra)
            _safe_publish("robot/virtual_test_event", payload)
            try:
                app = self.parent.winfo_toplevel()
                if hasattr(app, "update_virtual_test_progress"):
                    app.update_virtual_test_progress(exec_robot, virtual_session_id, event, cycle_index, virtual_target_cycles, status)
            except Exception:
                pass

        def _dry_event(kind, label="", payload=None):
            if not dry_run:
                return
            event = {
                "session_id": virtual_session_id,
            ...
            _safe_publish("robot/dry_run_event", event)

        def _set_program_do(idx, val, label="DO"):
            try:
                idx = int(idx)
                val = int(val)
            except (TypeError, ValueError):
                print(f">>   ⚠️ {label} 핀/값 변환 실패: idx={idx}, val={val}")
                return
            if dry_run:
                print(f">>   [DRY RUN] {label}{idx} = {'ON' if val else 'OFF'} 실제 출력 생략")
                _dry_event("do_bypass", f"{label}{idx}", {"idx": idx, "value": val})
                return
        ...
        def _publish_task_done(payload):
            ...
            topic = "robot/dry_run_task_done" if dry_run else "robot/task_done"
            _safe_publish(topic, payload)
        ...
        if not all_nodes:
            ...
            return
        with ProgramTreeEditor._execution_lock:
            if exec_robot in ProgramTreeEditor._running_robot_names:
                print(f">> [경고] {exec_robot} 프로그램이 이미 실행 중입니다. (중복 실행 차단)")
                return
            ProgramTreeEditor._running_robot_names.add(exec_robot)
        ...
        def _unhighlight(item_id):
            try:
                node_type = self.node_data.get(item_id, {}).get("__raw__", {}).get("type")
                self._retag_tree_item(item_id, node_type)
            except: pass

        def _abort_program(reason, stage="", item_id=None, ng=True):
            self._exec_stop = True
            RobotControlUseCase.request_stop(exec_robot)
            try:
                inst = _exec_inst()
                if inst:
                    inst.stop_motion()
                ...
                pass
            prefix = "[N.G]" if ng else "[중단]"
            stage_msg = f" ({stage})" if stage else ""
            print(f">> {prefix}{stage_msg} {reason} — 프로그램 정지")
            if ng:
                self._exec_ng_reason = reason
            try:
                if hasattr(self, "info_label"):
                    self.parent.after(0, lambda: self.info_label.configure(
                        text=f"N.G: {reason}",
                        text_color=Theme.DANGER,
                    ))
                if item_id:
                    self.tree.item(item_id, tags=("ng",))
                    self.tree.tag_configure("ng", background=Theme.DANGER, foreground="#FFFFFF")
            except Exception:
                pass
            raise _ProgramAbortException(reason)

        def _check_robot_fault(stage="", item_id=None):
            inst = _exec_inst()
            if not inst:
                return
            try:
                status = inst.get_robot_status()
                reason = RobotControlUseCase._fault_reason(status)
                if reason:
                    _abort_program(reason, stage, item_id, ng=True)
            except _ProgramAbortException:
                raise
            except Exception:
                pass

        def _wait_for_move_or_ng(timeout_sec=None, stage="", item_id=None):
            timeout_sec = max(float(timeout_sec or 0.0), float(self.program_motion_timeout_sec))
            if self._exec_stop:
                raise _ProgramAbortException("사용자 중단")
            ok = RobotControlUseCase.wait_for_move_finish(timeout_sec, exec_robot)
            if ok:
                return True
            if self._exec_stop:
                raise _ProgramAbortException("사용자 중단")
            _check_robot_fault(stage, item_id)
            _abort_program(f"이동 완료 대기 실패/타임아웃({timeout_sec}초)", stage, item_id, ng=True)

        def _scaled_vel_level(boundary=None, default_level=5):
            if isinstance(boundary, dict):
                level = boundary.get("velLevel", boundary.get("accLevel", default_level))
            else:
                level = default_level
            try:
                level = int(round(float(level) * self.pc_execution_speed_scale))
            except (TypeError, ValueError):
                level = int(round(default_level * self.pc_execution_speed_scale))
            return max(1, min(9, level))

        def _apply_motion_speed(boundary=None, is_joint=False, label=""):
            inst = _exec_inst()
            if not inst:
                return
            level = _scaled_vel_level(boundary)
            cache_key = "_last_joint_vel_level" if is_joint else "_last_task_vel_level"
            if getattr(self, cache_key, None) == level:
                return
            try:
                if is_joint and hasattr(inst, "set_joint_vel_level"):
                    inst.set_joint_vel_level(level)
                elif (not is_joint) and hasattr(inst, "set_task_vel_level"):
                    inst.set_task_vel_level(level)
                setattr(self, cache_key, level)
                motion_type = "Joint" if is_joint else "Task"
                print(f">>   [속도] {motion_type} velLevel={level} ({int(self.pc_execution_speed_scale*100)}% 적용)")
            except Exception as e:
                print(f">>   [속도 경고] {label} 속도 적용 실패: {e}")

        def _apply_pc_speed_limit():
            inst = _exec_inst()
            if not inst:
                return
            ratio = float(self.pc_execution_speed_scale * 100.0)
            print(f">> [속도] PC 직접 실행 안전속도 {ratio:g}% 적용")
            try:
                if hasattr(inst, "set_reduced_mode"):
                    inst.set_reduced_mode(True)
                if hasattr(inst, "set_reduced_speed_ratio"):
                    inst.set_reduced_speed_ratio(ratio)
            except Exception as e:
                print(f">> [속도 경고] Reduced speed 적용 실패: {e}")
            _apply_motion_speed({"velLevel": 5, "accLevel": 5}, is_joint=True, label="기본 Joint")
        ...
        def _apply_node_tcp(data, raw, stage="", item_id=None):
            ...
            try:
                ...
                _abort_program(f"TCP 적용 실패: {e}", stage, item_id, ng=True)
        ...
        def _get_variable_or_number(token):
            name = str(token)
            variables = RobotControlUseCase.get_all_variables(exec_robot)
            if name in variables:
                return variables[name]
            return _to_number(token, 0.0)

        def _eval_math_expr(expr):
            """Evaluate a tiny Conty variable expression such as Red+1 safely."""
            import ast
            ...
            def _eval(node):
                ...
                raise ValueError(f"지원하지 않는 수식: {expr}")

            return _eval(tree)

        def _apply_var_list(var_list, label="변수"):
            ...
            for entry in var_list:
                ...
                try:
                    value = _eval_math_expr(raw_value)
                except Exception as e:
        ...
        def _resolve_operand(operand):
            if isinstance(operand, dict):
                ...
                if operand.get("type", -1) == 10:
                    return _get_variable_or_number(value)
                return _to_number(value, 0.0)
            return _get_variable_or_number(operand)

        def _eval_conty_condition(cond):
            if not cond:
                return False
            left = _resolve_operand(cond.get("left", {}))
            right = _resolve_operand(cond.get("right", {}))
        ...
        def _wait_for_di_condition(di_list, label="Wait DI", item_id=None, timeout_sec=None):
            ...
            in_loop = loop_context.get("depth", 0) > 0
            effective_timeout = 0 if in_loop else timeout
            timeout_msg = "무한(Loop 내부)" if in_loop else ("무한" if timeout <= 0 else f"{timeout:g}s")
        ...
        def _execute_condition_chain(node_list, start_idx):
            ...
            executed = False
            for branch in chain:
                b_raw = branch["data"].get("__raw__", {})
                b_type = b_raw.get("type", -1)
                if b_type == 26:
                    result = True
                    label = "Else"
                else:
                    cond = branch["data"].get("cond", b_raw.get("cond", {}))
                    result = _eval_conty_condition(cond)
                    label = "If" if b_type == 24 else "Elif"
                cond_msg = "" if b_type == 26 else f" ({_condition_text(cond)})"
                print(f">>   🔀 {label}{cond_msg} → {'TRUE' if result else 'FALSE'}")
        ...
        def _execute_node_list(node_list):
            ...
            for idx, node in enumerate(node_list):
                _activate_exec_robot()
                if idx in skip_indices:
                    continue
                if self._exec_stop:
                    return
                if RobotControlUseCase.is_stop_requested(exec_robot):
                    raise _ProgramAbortException("정지 플래그 활성화")
                ...
                if "Main Program" in text or node_type == 999:
                    ...
                    try:
                        while not self._exec_stop:
                            ...
                            if pallet_size > 0:
                                ...
                                count_msg = f"/{effective}" if effective is not None else " (무한)"
                                print(f">> 🔄 Loop #{iteration}{count_msg} (팔레트 슬롯 {self._pallet_loop_idx + 1}/{pallet_size})")
                            else:
                    ...
                    print(">>   ⏹ Stop 노드 도달 — 프로그램 정지")
                    self._exec_stop = True
                    try:
                        RobotControlUseCase.stop_program(exec_robot)
                    except Exception as e:
                        print(f">>   ⚠️ Stop 명령 실패: {e}")
                    return
                    ...
                    for d in do_list:
                        idx = d.get("idx", 0)
                        val = d.get("value", 0)
                        _set_program_do(idx, val, label="DO")
                    ...
                    for d in do_list:
                        ...
                        val = d.get("value", 0)
                        if dry_run:
                            _dry_event("endtool_do_bypass", f"EndTool DO{idx}", {"idx": idx, "value": val})
                    ...
                    if has_children:
                        ...
                        result = False
                        if di_list:
                            result = _di_condition_met(di_list, "If DI")
                    ...
                    timeout = float(data.get("time", raw.get("time", 0)) or 0)
                    deadline = time.time() + timeout if timeout > 0 else None
                    print(f">>   ⏳ WaitFor 조건 대기 (timeout={timeout}s)")
                    while not self._exec_stop:
                        result = _eval_conty_condition(cond) if cond else False
                        if result:
                            print(">>   ✅ WaitFor 조건 충족")
                            break
                    ...
                    if cond:
                        ...
                        result = RobotControlUseCase.eval_condition(str(var_name), op_str, float(compare_val or 0), exec_robot)
                    ...
                    def _do_tool_action(do_hold):
                        ...
                        if do_map:
                            ...
                            for d in off_cmds:
                                try:
                                    if dry_run:
                                        _dry_event("tool_do_bypass", action_name, {"idx": d["idx"], "value": 0})
                                        print(f">>     [DRY RUN] DO{d['idx']}=OFF ({action_name}) 실제 출력 생략")
                                    else:
                            ...
                            for d in on_cmds:
                                try:
                                    if dry_run:
                                        _dry_event("tool_do_bypass", action_name, {"idx": d["idx"], "value": 1})
                                        print(f">>     [DRY RUN] DO{d['idx']}=ON ({action_name}) 실제 출력 생략")
                                    else:
                                ...
                                if True:
                                    ...
                                    if partner_node and between_nodes:
                                        for bn in between_nodes:
                                            ...
                                            if bn_type == 100:
                                                print(f">>   → Home 이동")
                                                _apply_motion_speed({"velLevel": 5, "accLevel": 5}, is_joint=True, label="Home")
                                                RobotControlUseCase.go_home(exec_robot)
                                                _wait_for_move_or_ng(stage=text, item_id=item_id)
                                            elif bn_type in (1, 102, 103):
                                                bn_q = bn["data"].get("q", [0.0]*6)
                                                bn_p = bn["data"].get("p", [0.0]*6)
                                                bn_boundary = bn["data"].get("boundary", bn_raw.get("boundary", {"velLevel": 5, "accLevel": 5}))
                                                if bn_type == 1:
                                                    _apply_motion_speed(bn_boundary, is_joint=True, label=bn["text"])
                                                    inst.joint_move_to(bn_q)
                                                else:
                                                    _apply_motion_speed(bn_boundary, is_joint=False, label=bn["text"])
                ...
                elif node_type == 30:  # Elif(DI)
                    print(f">>   🔀 Elif(DI) 단독 실행은 조건 체인 리팩터링 대상 — 자식 스킵")

                elif node_type in [32, 250]:  # SpeedRatio
                    spd = data.get("prgSpdRatio", raw.get("prgSpdRatio", 100))
                    try:
                        scaled_spd = max(1, min(100, int(round(float(spd) * self.pc_execution_speed_scale))))
                    except (TypeError, ValueError):
                        scaled_spd = int(self.pc_execution_speed_scale * 100)
                    print(f">>   ⚡ 프로그램 속도 변경: {spd}% → PC 안전속도 {scaled_spd}%")
                    try:
                        inst = _exec_inst()
                        if inst and hasattr(inst, 'set_reduced_mode'):
                            inst.set_reduced_mode(True)
                        if inst and hasattr(inst, 'set_reduced_speed_ratio'):
                            inst.set_reduced_speed_ratio(float(scaled_spd))
                        elif inst and hasattr(inst, 'set_speed_ratio'):
                            inst.set_speed_ratio(scaled_spd)
                    except Exception as e:
                ...
                elif node_type == 901:  # Call SubProgram placeholder
                    sub_path = raw.get("sub_program", data.get("sub_program", ""))
                    if sub_path:
                        print(f">>   📞 서브프로그램: {sub_path}")
                        RobotControlUseCase.call_sub_program(sub_path, exec_robot)
                        _wait_for_move_or_ng(stage=text, item_id=item_id)

                elif node_type == 302:  # TaktTime
                    print(f">>   ⏱ TaktTime 노드 (native JSON 실행 우선)")

                elif node_type == 902:  # Force placeholder
                    print(f">>   💪 Force 노드 (APK 타입 미확정 — 직접 실행 스킵)")
                    ...
                    if "Stack Search" in text:
                        ...
                        if found_pos:
                            RobotControlUseCase.set_variable("stack_z", found_pos[2], exec_robot)
        ...
        def _clear_all_highlights():
            ...
            def _clear(parent):
                for child in self.tree.get_children(parent):
                    try:
                        node_type = self.node_data.get(child, {}).get("__raw__", {}).get("type")
                        self._retag_tree_item(child, node_type)
                    except: pass
                    _clear(child)
            try: _clear("")
            except: pass
        ...
        def _run():
            ...
            log_path = _os.path.join(log_dir, f"exec_{exec_robot}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
            log_lines = []

            def _set_app_running(is_running):
                try:
                    app = self.parent.winfo_toplevel()
                    if hasattr(app, "set_program_running"):
                        app.set_program_running(exec_robot, is_running)
                    elif hasattr(app, "_program_running"):
                        app._program_running = is_running
                except Exception:
                    pass

            _set_app_running(True)
            completed_cycles = 0
            try:
                start_msg = f">> 🚀 {exec_robot} 프로그램 실행을 시작합니다!"
                ...
                try:
                    ...
                    while not self._exec_stop:
                        ...
                        if not is_virtual_test:
                            break
                except _ProgramAbortException as e:
                    if not self._exec_ng_reason and str(e) != "사용자 중단":
                        print(f">> [중단] {e}")
                        log_lines.append(f">> [중단] {e}")
                except Exception as e:
                    print(f">> [실행 에러] {e}")
                ...
                if getattr(self, "_exec_ng_reason", None):
                    result_msg = f">> [N.G] {exec_robot} 프로그램이 정지되었습니다: {self._exec_ng_reason}"
                    virtual_status = "ng"
                elif self._exec_stop:
                    result_msg = f">> ⏹ {exec_robot} 사용자에 의해 프로그램이 중단되었습니다."
                    virtual_status = "stopped"
                else:
                    result_msg = f">> ✅ {exec_robot} 프로그램 실행 완료!"
                    virtual_status = "completed"
                print(result_msg)
                log_lines.append(result_msg)
                _notify_virtual("session_done", completed_cycles, virtual_status, {"result": result_msg})
            finally:
                self._virtual_cycle_index = 0
                _set_app_running(False)
                with ProgramTreeEditor._execution_lock:
                    ProgramTreeEditor._running_robot_names.discard(exec_robot)
                try:
                    with open(log_path, 'w', encoding='utf-8') as f:
                        f.write("\n".join(log_lines))
                    print(f">> [로그] 실행 요약 저장됨: {log_path}")
                except Exception as e:
                    print(f">> [로그 에러] {e}")
    ...
    def _copy_node(self):
        ...
        parent = self.tree.parent(item)
        idx = self.tree.index(item)
        node_type = self.node_data.get(item, {}).get("__raw__", {}).get("type")
        new_item = self._insert_tree_item(parent, idx + 1, text=item_text, node_type=node_type)
        ...
        def _copy_children(src, dst):
            for child in self.tree.get_children(src):
                ...
                child_type = self.node_data.get(child, {}).get("__raw__", {}).get("type")
                new_child = self._insert_tree_item(dst, text=child_text, node_type=child_type)
                if child in self.node_data:
    ...
    def add_node(self, cmd_name):
        ...
        internal_name = command.label if command else cmd_name
        conty_type = command.type_id if command else 100

        selected = self.tree.selection()
        if not selected:
            new_item = self._insert_tree_item("", text=f" {internal_name} Node", node_type=conty_type)
        ...
        if is_container:
            new_item = self._insert_tree_item(target, text=f" {internal_name} Node", node_type=conty_type)
            self.tree.item(target, open=True)
        else:
            parent = self.tree.parent(target)
            idx = self.tree.index(target)
            new_item = self._insert_tree_item(parent, idx + 1, text=f" {internal_name} Node", node_type=conty_type)
    ...
    def _auto_teach(self, new_item, cmd_name, conty_type=100):
        ...
        raw = {"type": conty_type, "enable": True, "pId": 0}
        if conty_type == 1:
            pass
        elif conty_type == 3:
            raw["varList"] = [{"name": "var1", "type": 1, "value": 0}]
        elif conty_type == 23:
            raw.update({
                "time": 0,
                "cond": {
            ...
            raw["toolCmd"] = {"toolId": 0, "cmdId": 0}
        elif conty_type == 41:
            raw.update({"toolCmd": {"toolId": 0, "cmdId": 0}, "sensName": "sensor1"})
        elif conty_type == 50:
            raw.update({"scriptFile": "script.py", "scriptCode": ""})
        elif conty_type == 250:
            raw["prgSpdRatio"] = 100
        elif conty_type == 302:
            raw.update({"careTackTime": 0, "targetTakt": 10.0})
        elif conty_type == 901:
            raw["subProgram"] = "sub_routine.json"
        ...
        if "prgSpdRatio" in raw:
            self.node_data[new_item]["prgSpdRatio"] = raw["prgSpdRatio"]
        if "varList" in raw:
            self.node_data[new_item]["varList"] = raw["varList"]
        if "cond" in raw:
            self.node_data[new_item]["cond"] = raw["cond"]
            self.node_data[new_item]["time"] = raw["time"]
        self.tree.selection_set(new_item)
...
class OptionsEditor:
    def __init__(self, parent_frame, robot_name_provider=None):
        ...
        self.selected_robot = robot_manager.get_active_robot_name() or "Robot A"
        self.polling = False

    def _robot_names(self):
        names = list(robot_manager.get_all_robots().keys())
        return names or ["Robot A", "Robot B", "Robot C"]
        ...
        if not robot:
            try:
                if callable(self.robot_name_provider):
                    robot = self.robot_name_provider()
            except Exception:
                robot = None
        robot = robot or self.selected_robot or "Robot A"
    ...
    def _on_robot_changed(self, robot_name):
        ...
        self.load_tcp_from_program(silent=True)
        print(f">> [옵션] 제어 대상 로봇 선택: {robot_name}")

    def _update_selected_status(self, text=None, color=None):
        if not hasattr(self, "robot_status_label"):
            return
        robot_name = self._get_selected_robot_name()
        inst = self._selected_robot_instance(robot_name)
        if text is None:
            text = f"{robot_name} 연결됨" if inst else f"{robot_name} 연결 안됨"
        if color is None:
            color = "#00FF41" if inst else Theme.DANGER
        self.robot_status_label.configure(text=text, text_color=color)

    def _set_entry_value(self, entry, value):
        entry.delete(0, "end")
        entry.insert(0, f"{value:g}" if isinstance(value, (int, float)) else str(value))

    def _check_robot_settings(self, silent=False):
        robot_name = self._sync_selected_robot()
        inst = self._selected_robot_instance(robot_name)
        if not inst:
            msg = f"{robot_name}: 연결 안됨"
            if hasattr(self, "settings_summary"):
                self.settings_summary.configure(text=msg, text_color=Theme.DANGER)
            self._update_selected_status(msg, Theme.DANGER)
            if not silent:
                print(f">> [세팅 확인] {msg}")
            return False
        ...
        details = []
        try:
            status = inst.get_robot_status() if hasattr(inst, "get_robot_status") else {}
            if status:
                active_flags = [
                ...
                details.append("상태: " + (", ".join(active_flags) if active_flags else "Idle"))
        except Exception as e:
            details.append(f"상태 읽기 실패: {e}")

        try:
            tcp = _coerce_tcp(inst.get_default_tcp()) if hasattr(inst, "get_default_tcp") else None
            if tcp:
                display_tcp = [tcp[0] * 1000.0, tcp[1] * 1000.0, tcp[2] * 1000.0, tcp[3], tcp[4], tcp[5]]
                labels = ["X (mm)", "Y (mm)", "Z (mm)", "Rx (deg)", "Ry (deg)", "Rz (deg)"]
                for label, value in zip(labels, display_tcp):
                    self._set_entry_value(self.tcp_entries[label], value)
                details.append(f"TCP(mm): X={display_tcp[0]:g}, Y={display_tcp[1]:g}, Z={display_tcp[2]:g}")
        except Exception as e:
            details.append(f"TCP 읽기 실패: {e}")

        if self.load_tool_mapping_from_program(silent=True):
            try:
                details.append(
                    f"Tool DO: grip={self.grip_pin_entry.get()}, release={self.release_pin_entry.get()}"
                )
            except Exception:
                pass

        for method, label in [
            ("get_collision_level", "충돌Lv"),
            ("get_joint_vel_level", "JointVel"),
            ("get_task_vel_level", "TaskVel"),
            ("get_joint_blend_radius", "JointBlend"),
            ("get_task_blend_radius", "TaskBlend"),
            ("get_reduced_mode", "Reduced"),
            ("get_reduced_speed_ratio", "ReducedRatio"),
        ]:
            try:
                if hasattr(inst, method):
                    details.append(f"{label}: {getattr(inst, method)()}")
            except Exception:
                pass

        summary = f"{robot_name} | " + " | ".join(details[:8])
        if hasattr(self, "settings_summary"):
            self.settings_summary.configure(text=summary, text_color=Theme.TEXT_PRIMARY)
        self._update_selected_status(f"{robot_name} 세팅 확인 완료", "#00FF41")
        if not silent:
            print(f">> [세팅 확인] {summary}")
        return True
    ...
    def render(self):
        ...
        control_bar = ctk.CTkFrame(f, fg_color=Theme.BG_SURFACE, corner_radius=8)
        control_bar.pack(fill="x", padx=10, pady=(0, 10))
        ctk.CTkLabel(control_bar, text="대상 로봇", font=Theme.font(size=13, weight="bold")).pack(side="left", padx=(15, 8), pady=8)
        self.robot_sel = ctk.CTkOptionMenu(
            control_bar,
            values=self._robot_names(),
            width=120,
            command=self._on_robot_changed,
            fg_color=Theme.BG_BASE,
            button_color=Theme.ACCENT_PRIMARY,
        )
        self.robot_sel.set(self.selected_robot)
        self.robot_sel.pack(side="left", padx=5, pady=8)
        self.robot_status_label = ctk.CTkLabel(control_bar, text="", font=Theme.font(size=12, weight="bold"))
        self.robot_status_label.pack(side="left", padx=12)
        ctk.CTkButton(control_bar, text="🔎 세팅값 확인", width=110, height=28,
                      command=self._check_robot_settings,
                      **Theme.get_button_style("secondary")).pack(side="right", padx=10, pady=8)
        self.settings_summary = ctk.CTkLabel(control_bar, text="", font=Theme.font(size=11),
                                             text_color=Theme.TEXT_SECONDARY, anchor="w")
        self.settings_summary.pack(side="right", fill="x", expand=True, padx=8)
        self._sync_selected_robot()
        self._update_selected_status()
        ...
        ctk.CTkButton(tcp_frame, text="📥 JSON TCP 불러오기", fg_color=Theme.INFO, hover_color="#1565C0", height=38, font=Theme.font(size=14, weight="bold"), command=self.load_tcp_from_program).pack(pady=(8, 0), padx=20, fill="x")
        ...
        ctk.CTkButton(tool_frame, text="📥 JSON 툴 매핑 불러오기", fg_color=Theme.INFO, hover_color="#1565C0", height=36, font=Theme.font(size=13, weight="bold"), command=self.load_tool_mapping_from_program).pack(pady=(0, 8), padx=20, fill="x")
        self.load_tool_mapping_from_program(silent=True)
    ...
    def toggle_do(self, idx, btn):
        ...
        inst = self._selected_robot_instance(robot_name)
        if not inst:
            print(f">> [경고] {robot_name} 로봇이 연결되지 않았습니다.")
            return
        ...
        from core.domains.robot.use_cases.robot_control_usecase import RobotControlUseCase
        RobotControlUseCase.set_do(idx, new_val, robot_name)
        print(f">> [D.O 제어] {robot_name} DO 핀 {idx}을(를) {new_val} 상태로 변경했습니다.")

    def poll_io(self):
        while self.polling:
            name = self._get_selected_robot_name()
            inst = self._selected_robot_instance(name)
            if inst and name:
                try:
                    # 타이틀 업데이트
                    ...
                    self.parent.after(0, self._update_selected_status)
                ...
                try:
                    self.parent.after(0, lambda n=name: _safe_configure(self.di_title, text=f"📥 Digital Input (D.I) - {n} 연결 안됨"))
                    self.parent.after(0, lambda n=name: _safe_configure(self.do_title, text=f"📤 Digital Output (D.O) - {n} 연결 안됨"))
                    self.parent.after(0, self._update_selected_status)
                except: pass
            time.sleep(0.5)
    ...
    def apply_tcp(self):
        ...
        inst = self._selected_robot_instance(robot_name)
        if not inst:
            print(f">> [경고] TCP를 적용할 {robot_name} 로봇이 연결되지 않았습니다.")
            return
        ...
        try:
            ...
            from core.domains.robot.use_cases.robot_control_usecase import RobotControlUseCase
            RobotControlUseCase.set_tcp(tcp_vals, robot_name)
            print(f">> [TCP 설정] {robot_name} 화면 입력(mm)을 로봇값(m)으로 변환해 적용: {tcp_vals}")
        except ValueError:
            print(">> [에러] TCP 입력란에는 숫자만 입력해주세요.")

    def _current_program_path(self):
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
        robot_name = self._get_selected_robot_name()
    ...
    def load_tcp_from_program(self, silent=False):
        ...
        try:
            ...
            if not tcp:
                if not silent:
                    print(">> [TCP] JSON에서 TCP 값을 찾지 못했습니다.")
                return False
            labels = ["X (mm)", "Y (mm)", "Z (mm)", "Rx (deg)", "Ry (deg)", "Rz (deg)"]
            display_tcp = [tcp[0] * 1000.0, tcp[1] * 1000.0, tcp[2] * 1000.0, tcp[3], tcp[4], tcp[5]]
            for label, value in zip(labels, display_tcp):
                ent = self.tcp_entries[label]
                ent.delete(0, "end")
                ent.insert(0, f"{value:g}")
            if not silent:
                print(f">> [TCP] JSON TCP {tcp} → 화면 표시(mm) {display_tcp}")
            return True
        except Exception as e:
            if not silent:
                print(f">> [TCP 오류] JSON TCP 불러오기 실패: {e}")
            return False

    def load_tool_mapping_from_program(self, silent=False):
        path = self._current_program_path()
        if not os.path.exists(path):
            if not silent:
        ...
        try:
            ...
            cfg_node = None
            for node in data.get("program", []):
                if isinstance(node, dict) and node.get("type") == 999:
                    cfg_node = node
                    break
            tool_info = (cfg_node or {}).get("toolInfo", [])
            if not tool_info:
                if not silent:
                    print(">> [툴 매핑] JSON에서 toolInfo를 찾지 못했습니다.")
                return False

            tool = tool_info[0]
            hold_pin = None
            release_pin = None
            for cmd in tool.get("toolCommand", []):
                name = str(cmd.get("name", "")).lower()
                do_map = cmd.get("doMap") or []
                active_pin = None
                for item in do_map:
                    if item.get("value", 1) == 1:
                        active_pin = item.get("idx")
                        break
                if active_pin is None and do_map:
                    active_pin = do_map[0].get("idx")
                if active_pin is None:
                    continue
                if "hold" in name or "grip" in name or "pick" in name:
                    hold_pin = active_pin
                elif "release" in name or "ungrip" in name or "place" in name:
                    release_pin = active_pin

            if hold_pin is not None:
                self._set_entry_value(self.grip_pin_entry, int(hold_pin))
            if release_pin is not None:
                self._set_entry_value(self.release_pin_entry, int(release_pin))
            if not silent:
                print(f">> [툴 매핑] JSON 로드: Grip/Hold DO={hold_pin}, Release DO={release_pin}")
            return hold_pin is not None or release_pin is not None
        except Exception as e:
            if not silent:
                print(f">> [툴 매핑 오류] JSON 툴 매핑 불러오기 실패: {e}")
            return False
    ...
    def _test_grip(self, action):
        ...
        inst = self._selected_robot_instance(robot_name)
        if not inst:
            print(f">> [경고] 테스트할 {robot_name} 로봇이 연결되지 않았습니다.")
            return
        ...
        try:
            ...
            def _bg():
                ...
                if is_suction:
                    if action == "grip":
                        print(f">> [흡착 테스트] {robot_name} 진공 ON (DO {g_pin} ON)")
                        RobotControlUseCase.set_do(g_pin, 1, robot_name)
                    else:
                        print(f">> [흡착 테스트] {robot_name} 진공 OFF (DO {g_pin} OFF)")
                        RobotControlUseCase.set_do(g_pin, 0, robot_name)
                else:
                    if action == "grip":
                        print(f">> [그리퍼 테스트] {robot_name} Grip 작동 (DO {g_pin} ON, DO {r_pin} OFF)")
                        RobotControlUseCase.set_do(r_pin, 0, robot_name)
                        time.sleep(0.1)
                        RobotControlUseCase.set_do(g_pin, 1, robot_name)
                    else:
                        print(f">> [그리퍼 테스트] {robot_name} Release 작동 (DO {g_pin} OFF, DO {r_pin} ON)")
                        RobotControlUseCase.set_do(g_pin, 0, robot_name)
                        time.sleep(0.1)
