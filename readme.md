...
### 🧱 팔레타이징 (Pick & Place)
- **Pick/Place 인터리빙**: 팔레트의 포인트를 한 번에 모두 집지 않고, P1 잡고 → Place 1 놓고 → P2 잡고 → Place 2 놓는 실제 공정 방식(인터리빙) 구현
- **6가지 배치 패턴**: 일반(Z형), 지그재그, 교차(90°), S자형, 외곽나선, 중앙확산
- **마법사 ↔ 에디터 양방향 연동**: 마법사에서 디자인 → 에디터에 M/N/L, 크기, 간격, 패턴 자동 반영
- **P1 기반 자동 좌표 계산**: P2(행 끝), P3(열 끝), P4(층 끝) 미설정 시 자동 계산(Z축 보정 포함)
- **3동작 시퀀스**: Approach → Target(Hold/Release) → Retract 자동 실행

### 💾 외부 MES 연동 (DB 저장)
- **실시간 상태 기록**: `pymysql`을 이용해 각 로봇의 6축 관절 각도(q) 및 토크(torque)를 10Hz 폴링으로 DB에 UPSERT (한 로봇당 최신 상태 유지)
- **작업 완료 추적**: Pick, Place 작업 시 해당 위치의 XYZ 좌표를 이력(History) 테이블에 누적 저장
- **Persistent Connection**: 커넥션 풀링/재사용 기법을 통해 100ms 간격 통신 시의 부하 최소화
